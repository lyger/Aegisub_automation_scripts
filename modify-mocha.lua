...
local function style_lookup(line)
	local style_table={
		...
		["blur"] = 0,
		["be"] = 0,
		["fn"] = line.styleref.fontname,
		["b"] = line.styleref.bold and 1 or 0,
		["i"] = line.styleref.italic and 1 or 0,
		["u"] = line.styleref.underline and 1 or 0,
		["s"] = line.styleref.strikeout and 1 or 0,
		["an"] = line.styleref.align
	}
	return style_table
end
...
local function make_full_state_table(line_table)
	...
	for i,val in ipairs(line_table) do
		this_state_table[i]={}
		local pstate=line_exclude(val.tag,global_excludes)
...
function modify_mocha(sub,sel)
	...
	for si,li in ipairs(sel) do
		checkline=sub[li]
		if checkline.actor:lower():find("^orig$")~=nil then
			oline=sub[li]
			oindex=li
		elseif checkline.actor:lower():find("^mod$")~=nil then
			mline=sub[li]
			mindex=li
		end
		if oline~=nil and mline~=nil then break end
	end
	
	if oline==nil or mline==nil then
	...
	--Preprocess
	karaskel.preproc_line(sub,meta,styles,oline)
	karaskel.preproc_line(sub,meta,styles,mline)
	...
	otable,mtable=match_splits(otable,mtable)
	...
	o_state_table=make_full_state_table(otable)
	m_state_table=make_full_state_table(mtable)
	...
	ostyle=style_lookup(oline)
	mstyle=style_lookup(mline)
	...
	for si,li in ipairs(sel) do
		if li~=mindex and li~=oindex then
			...
			--Make deep copy of mtable
			local mtable_copy = {}
			for i,val in ipairs(mtable) do
				mtable_copy[i]={tag=val.tag,text=val.text}
			end
			
			--Make deep copy of delta state table
			local delta_state_copy = {}
			for i,val in ipairs(delta_state_table) do
				delta_state_copy[i]={}
				for ctag,cparam in pairs(val) do
					delta_state_copy[i][ctag]=cparam
				end
			end
			...
			while(j<=#mtable_copy) do
				mtext=mtable_copy[j].text
				mtag=mtable_copy[j].tag
			...
			this_state_table=make_full_state_table(this_table)
			...
			for i,tval in ipairs(this_table) do
				...
				rebuilt_line=rebuilt_line..tval.tag..tval.text
			end
			
			--Re-insert
			this_line.text=rebuilt_line:gsub("{}","")
...
aegisub.register_macro(script_name,script_description,modify_mocha)
