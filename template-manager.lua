...
next_menu=nil
menu_params=function() return nil end

--Global variables for subtitles and selected lines
sub,sel=nil,nil

--List of template groups
tglist={}

--Name of current template group
ctg=nil
...
--Return the keys of a table as a list
function keys(t)
	local kt={}
	for k in pairs(t) do
		table.insert(kt,k)
...
function fill_rows(conf,idx1,idx2)
	...
	for i=idx1,idx2 do
		...
		l,tt,st,ed,sty,ln=parse(data[ctemp][i])
...
function mod_temp()
	...
	--Display dialog
	pr,rs=aegisub.dialog.display(conf,opts,opts_ids)
			...
			local npressed,nresults=aegisub.dialog.display(
				{
					...
					{x=0,y=1,width=7,height=1,class="edit",name="tname",
						value="Template "..next_n(keys(data),"Template")}
